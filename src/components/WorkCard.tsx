...
export default function WorkCard(work: Work) {
  return (
    <HoverCard.Root>
      <HoverCard.Trigger>
        <div className="flex group items-center cursor-pointer bg-zinc-800 w-full p-8 space-x-4 rounded-2xl">
          <img className="size-10" src={work.logo} alt={work.title} />
          <h2 className="font-bold group-hover:text-slate-50 transition text-2xl">{work.title}</h2>
        </div>
        <HoverCard.Portal>
          <HoverCard.Content
